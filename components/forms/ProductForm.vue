...
import type { IFormField } from "~/types/product";
...
const createProduct = async () => {
  ...
  formCreateProduct.forEach((item: IFormField) => {
    if (item.name) {
    ...
    <div class="w-[500px]">
      ...
      <form @submit.prevent="createProduct">
        <div class="mb-6" v-for="(item, index) in formCreateProduct" :key="index">
          ...
          <select v-if="item.type === 'select'" :id="item.name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" v-model="item.value">
            <option disabled value="">Choose Category</option>
            <option v-for="(category, index) in categories" :key="index" :value="category.name">{{ capitalizeFirstLetter(category.name) }}</option>
          </select>
          <input
